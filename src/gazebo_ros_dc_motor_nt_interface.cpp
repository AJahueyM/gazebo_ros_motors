...
#include "gazebo_ros_dc_motor_nt_interface/gazebo_ros_dc_motor_nt_interface.h"

namespace gazebo {

// Constructor
    GazeboRosMotorNT::GazeboRosMotorNT() {
    }
...
// Load the controller
    void GazeboRosMotorNT::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
        this->parent = _parent;
        ...
        gazebo_ros_->getParameter<double>(electromotive_force_constant_, "electromotive_force_constant",
                                          0.08); // Datasheet: 1.8Nm / 22A or 24V / 300 (rad/s)
        ...
        gazebo_ros_->getParameter<double>(nt_velocity_noise_, "nt_velocity_noise", 0.0);
        // gearbox parameters
        ...
        if (this->publish_motor_joint_state_) {
            joint_state_publisher_ = gazebo_ros_->node()->advertise<sensor_msgs::JointState>(
                    joint_->GetName() + "/joint_state", 1000);
...
//                command_topic_,
//                1,
//                boost::bind(&GazeboRosMotorNT::cmdVelCallback, this, _1),
//                ros::VoidPtr(),
//                &queue_
//        );
...
//        ROS_INFO_NAMED(plugin_name_, "%s: Subscribed to %s", gazebo_ros_->info(), command_topic_.c_str());
        ...
        ros::SubscribeOptions sov = ros::SubscribeOptions::create<std_msgs::Float32>(
                supply_topic_,
                1,
                boost::bind(&GazeboRosMotorNT::supplyVoltageCallBack, this, _1),
                ros::VoidPtr(),
                &queue_
        );
        ...
        if (this->publish_velocity_) {
            ...
            ROS_INFO_NAMED(plugin_name_, "%s: Advertising motor shaft (before gearbox) velocity on %s ",
                           gazebo_ros_->info(), velocity_topic_.c_str());
        }
        if (this->publish_encoder_) {
            encoder_publisher_ = gazebo_ros_->node()->advertise<std_msgs::Int32>(encoder_topic_, 1);
            ROS_INFO_NAMED(plugin_name_, "%s: Advertising encoder counts on %s ", gazebo_ros_->info(),
        ...
        if (this->publish_current_) {
            ...
            ROS_INFO_NAMED(plugin_name_, "%s: Advertising actual motor current on %s ", gazebo_ros_->info(),
                           current_topic_.c_str());
        }

        // listen to the update event (broadcast every simulation iteration)
        this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
                boost::bind(&GazeboRosMotorNT::UpdateChild, this));
        ...
        dynamic_reconfigure_server_.reset(
                new dynamic_reconfigure::Server<gazebo_ros_motors::motorModelConfig>(reconf_mutex_,
        ...
        voltageCommandEntry = motorTable->GetEntry("Voltage");
        currentEntry = motorTable->GetEntry("Current");
    ...
    bool GazeboRosMotorNT::checkParameters() {
    ...
    bool GazeboRosMotorNT::ValidateParameters() {
    ...
    void GazeboRosMotorNT::paramServerUpdate() {
        if (notify_server_) {
    ...
    void GazeboRosMotorNT::reconfigureCallBack(const gazebo_ros_motors::motorModelConfig &config, uint32_t level) {
        ...
        if (this->checkParameters()) {
            ...
            if (electric_resistance_ != config.electric_resistance) {
                ...
                if (!this->ValidateParameters()) {
                    ...
                    ROS_WARN_NAMED(plugin_name_,
                                   "Electric resistance %6.3f discarded, keeping previous value of %6.3f Ohm",
                                   config.electric_resistance, electric_resistance_);
                    ok = false;
                }
            }
            ...
            if (electric_inductance_ != config.electric_inductance) {
                ...
                if (!this->ValidateParameters()) {
                    ...
                    ROS_WARN_NAMED(plugin_name_,
                                   "Electric inductance %6.3f discarded, keeping previous value of %6.3f H",
                                   config.electric_inductance, electric_inductance_);
                    ok = false;
                }
            }
            ...
            if (moment_of_inertia_ != config.moment_of_inertia) {
                ...
                if (!this->ValidateParameters()) {
                    ...
                    ROS_WARN_NAMED(plugin_name_,
                                   "Moment of inertia %6.3f discarded, keeping previous value of %6.3f kgm^2",
                                   config.moment_of_inertia, moment_of_inertia_);
                    ok = false;
                }
            }
            ...
            if (armature_damping_ratio_ != config.armature_damping_ratio) {
                ...
                if (!this->ValidateParameters()) {
                    ...
                    ROS_WARN_NAMED(plugin_name_,
                                   "Armature damping %6.3f discarded, keeping previous value of %6.3f Nm/(rad/s)",
                                   config.armature_damping_ratio, armature_damping_ratio_);
                    ok = false;
                }
            }
            ...
            if (electromotive_force_constant_ != config.electromotive_force_constant) {
                ...
                if (!this->ValidateParameters()) {
                    ...
                    ROS_WARN_NAMED(plugin_name_,
                                   "Motor constant %6.3f discarded, keeping previous value of %6.3f Nm/(rad/s)",
                                   config.electromotive_force_constant, electromotive_force_constant_);
                    ok = false;
                }
            }
    ...
    void GazeboRosMotorNT::publishWheelJointState(double velocity, double effort) {
        if (this->publish_motor_joint_state_) {
...
// Velocity publisher
    void GazeboRosMotorNT::publishRotorVelocity(double m_vel) {
...
// Simple incremental encoder emulation
    void GazeboRosMotorNT::publishEncoderCount(double m_vel, double dT) {
    ...
    void GazeboRosMotorNT::publishMotorCurrent() {
...
// Motor Model update function
    void GazeboRosMotorNT::motorModelUpdate(double dt, double output_shaft_omega, double actual_load_torque) {
        if (input_ > 1.0) {
            input_ = 1.0;
        } else if (input_ < -1.0) {
            input_ = -1.0;
        ...
        double i_t = (emA * (i0 * (Km2 + d * R) * (d * L * (d * eOp1 * L + eOm1 * Om) + eOp1 * J2 * R2 -
                                                   J * (4 * eOp1 * Km2 * L + 2 * d * eOp1 * L * R + eOm1 * Om * R)) -
                             d * L * (d * (-2 * eA + eOp1) * L + eOm1 * Om) * (Km * T + d * V) -
                             (-2 * eA + eOp1) * J2 * R2 * (Km * T + d * V) + J * (Km3 * (-2 * eOm1 * o0 * Om +
                                                                                         4 * (-2 * eA + eOp1) * L * T) -
                                                                                  Km * R * (2 * d * eOm1 * o0 * Om -
                                                                                            2 * d * (-2 * eA + eOp1) *
                                                                                            L * T + eOm1 * Om * T) +
        ...
        double o_t = (emA * (-4 * eOp1 * J * pow(Km, 4) * L * o0 +
                             J * Km2 * R * (-6 * d * eOp1 * L * o0 + eOm1 * o0 * Om - 4 * (-2 * eA + eOp1) * L * T) +
                             J * R2 * (-2 * d2 * eOp1 * L * o0 + d * eOm1 * o0 * Om - 2 * d * (-2 * eA + eOp1) * L * T +
                                       eOm1 * Om * T) + 4 * (-2 * eA + eOp1) * J * Km3 * L * V -
                             J * Km * (-2 * d * (-2 * eA + eOp1) * L + eOm1 * Om) * R * V + J2 * R2 * (eOp1 * Km2 * o0 +
                                                                                                       d * eOp1 * o0 *
                                                                                                       R + (-2 * eA +
                                                                                                            eOp1) * R *
                                                                                                           T -
...
// Finalize the controller
    void GazeboRosMotorNT::FiniChild() {
        queue_.clear();
        queue_.disable();
...
// Callback from custom que
    void GazeboRosMotorNT::cmdVelCallback(const std_msgs::Float32::ConstPtr &cmd_msg) {
        input_ = cmd_msg->data;
    }

    void GazeboRosMotorNT::supplyVoltageCallBack(const std_msgs::Float32::ConstPtr &voltage) {
        supply_voltage_ = voltage->data;
    }

    void GazeboRosMotorNT::QueueThread() {
    ...
    void GazeboRosMotorNT::publishNT() {
